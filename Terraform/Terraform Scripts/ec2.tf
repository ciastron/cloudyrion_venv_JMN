...
resource "aws_instance" "vampi_api" {
  # ec2 have plain text secrets in user data
  ami           = "${var.ami}"
  instance_type = "t2.nano"
...
sudo apt-get update
sudo apt-get install docker docker-cli docker-ce docker-ce-cli containerd.io docker-engine docker.io containerd runc docker-compose-plugin
sudo git clone https://github.com/erev0s/VAmPI.git
cd VAmPI
sudo docker build -t vampi_docker:latest .
sudo docker run -p 5000:5000 vampi_docker:latest
EOF
}
